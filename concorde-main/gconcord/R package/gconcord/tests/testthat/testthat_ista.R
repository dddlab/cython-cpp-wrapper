...
omega.s = gconcord(S = S, lambda1 = 0.2, lambda2 = 0.05, method = "ista")
omega.x = gconcord(data = x, lambda1 = 0.2, lambda2 = 0.05, method = "ista")
...
expect_equal(sum(abs(omega.s-res)), 4e-07)  ## if estimated matrix is the same
expect_equal(sum(abs(omega.x-res)), 4e-07)  ## if estimated matrix is the same

## Test: ista cross validation
set.seed(1)
