...
omega.s = gconcord(S = S, lambda1 = 0.2, lambda2 = 0.05, method = "fista")
omega.x = gconcord(data = x, lambda1 = 0.2, lambda2 = 0.05, method = "fista")
...
expect_equal(sum(abs(omega.s-res)), 3.7e-07)  ## if estimated matrix is the same
expect_equal(sum(abs(omega.x-res)), 3.7e-07)  ## if estimated matrix is the same

## Test: fista cross validation
set.seed(1)
