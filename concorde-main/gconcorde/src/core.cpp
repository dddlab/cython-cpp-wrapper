              ...
              )
{
  ...
  MatrixXd subg(p, p);
  MatrixXd tmp(p, p);
  MatrixXd tmpdiagM(p, p);

  double subgnorm, Xnorm;
  double tau;
  int itr = 0;
  int loop = 1;
  ...
  // just for test
  tau = 0.5013430510933962;

  while (loop != 0){
    tmp = MatrixXd(X) - tau*G;

    // diagonal update
    tmpdiagM = tmp.diagonal().asDiagonal();
    tmpdiagM = 0.5*(tmpdiagM + (tmpdiagM.cwiseProduct(tmpdiagM).array() + 4.0*tau).cwiseSqrt().matrix());
    tmp.diagonal() = tmpdiagM.diagonal();

    // soft-thresholding
    sthreshmat(tmp, 1.0, LambdaMat);
    X = tmp.sparseView();

    G = X * S;

    if (lambda2 > 0) { G += lambda2 * 2 * MatrixXd(X); }

    tmp = MatrixXd(X).array().unaryExpr(ptr_fun(sgn));
    tmp = G + tmp.cwiseProduct(LambdaMat);
    subg = G;
    sthreshmat(subg, 1.0, LambdaMat);
    subg = (MatrixXd(X).array() != 0).select(tmp, subg);
    subgnorm = subg.norm();
    Xnorm = X.norm();

    itr += 1;

    loop = int((itr < maxitr) && (subgnorm/Xnorm > epstol));
